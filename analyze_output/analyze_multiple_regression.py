...
class MultiRegressionAnalysis:
    ...
    # plots errorbars for all models provided in file_paths list.
    ...
    # plots bias/residual vs variable (such as towall, visible energy, etc) in one plot
    def analyze_regression_multiple(self, settings):
        ...
        if settings.doML:
            ...
            for i, sub_dir in enumerate(sub_dir_names):
                ...
                print('mlPath', settings.mlPath)
                single_ml_analysis, multi_ml_analysis = analyze_ml_regression(settings)
                results.add_global_perf(f"ML_{i}", single_ml_analysis[0], single_ml_analysis[1], single_ml_analysis[2], single_ml_analysis[3], single_ml_analysis[4])
                results.add_var_perf(f"ML_{i}",multi_ml_analysis)
